...
test_that("Parallel Linear Model Estimate works", {
  ...
  # compare lm to its bootstrap estimate
  expect_equal(coef(lm_model), coef(blblm_model), tolerance = .001)
})
...
test_that("Sigma works", {
  ...
  expect_equal(sigma(blblm_model)[1],sigma(lm_model), tolerance = .001, ignore_attr = TRUE)
})
...
test_that("Confidence interval works", {
  ...
  expect_equal(confint(blblm_model),intervals, tolerance = .01, ignore_attr = TRUE)
})